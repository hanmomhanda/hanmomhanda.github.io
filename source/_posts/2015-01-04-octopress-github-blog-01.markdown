...
- octopress를 로컬에 clone하고(clone한 repo를 octopress repo라 한다)

- octopress repo 내에서 이런저런 설정을 하고

- GitHub의 Repository를 octopress repo의 remote로 설정

- octopress repo 내의 source 폴더 내에서 규약에 맞게 포스팅 할 블로그 파일 작성

- octopress repo를 remote로 push하면 GitHub의 Page 기능을 통해 블로그 내용이 웹 페이지로 반영
...
1. octopress repo를 로컬에 clone 후 bundler로 관련 프로그램 설치
...
GitHub 에서 이름이 GitHub_UserName.github.io 인 repository를 새로 생성한다.

나중에 이 repo에서 블로그 컨텐츠를 관리하게 된다.
...
2. 앞 단계에서 GitHub에 생성한 repository의 url 입력
...
2. 일반적인 Git commit/push 활용 
    ...
    `rake deploy`도 사실 Git 명령을 wrapping한 것에 불과하다(자세한 내용은 `Rakefile` 파일을 열어보면 알 수 있다). 당연히 직접 Git로도 가능한데, 게시물 작성 후 `rake generate`에 의해 .html로 변환된 내용이 들어있는 public를 _deploy 폴더로 복사한 후 git add/commit/push를 수행하면 된다.
...
다시 봐도, 아무리 기본이라지만 너무 허접하지 않은가..

2탄에서 여러가지 설정을 통해 허접한 기본 블로그를 봐줄 만한 블로그로 변신시켜보자.